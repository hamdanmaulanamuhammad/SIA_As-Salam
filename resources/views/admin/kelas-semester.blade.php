...
<div class="container px-5 mx-auto py-6">
    ...
    <div class="flex justify-between mb-6 mt-6">
        ...
        <div class="flex gap-2">
            <a href="{{ route('akademik.index') }}?tab=semester" class="text-sm px-4 py-2 text-white bg-gray-600 rounded-md hover:bg-gray-700">
    ...
    <div id="tambah-kelas-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 max-w-md max-h-screen overflow-y-auto">
            ...
            <form id="tambah-kelas-form" action="{{ route('akademik.kelas-semester.store', $semester->id) }}" method="POST">
                @csrf
    ...
    <div id="kelola-mapel-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 max-w-md max-h-screen overflow-y-auto">
            ...
            <form id="tambah-mapel-form" action="{{ route('akademik.kelas-semester.mapel.store') }}" method="POST">
                @csrf
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('tambah-kelas-button')?.addEventListener('click', () => {
        ...
        const actionUrl = "{{ route('akademik.kelas-semester.store', $semester->id) }}";
        document.getElementById('tambah-kelas-form').action = actionUrl;
        console.log('Opening kelas modal with action:', actionUrl);
    ...
    document.getElementById('tambah-kelas-form')?.addEventListener('submit', function(event) {
        ...
        const url = id ? "{{ url('akademik/kelas-semester') }}/" + id : "{{ route('akademik.kelas-semester.store', $semester->id) }}";
        const method = id ? 'PUT' : 'POST';
        const formData = new FormData(this);
        formData.append('_method', method);

        console.log('Submitting kelas form to:', url, 'with method:', method, 'data:', Object.fromEntries(formData));
        ...
        .catch(error => {
            console.error('Error submitting kelas form:', error);
    ...
    document.querySelectorAll('.edit-button').forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            const url = `{{ url('akademik/kelas-semester') }}/${id}/edit`;
            ...
            .then(response => {
                console.log('Edit response status:', response.status, 'Edit response URL:', response.url);
                if (!response.ok) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('tambah-kelas-form').action = `{{ url('akademik/kelas-semester') }}/${data.data.id}`;
        ...
        form.addEventListener('submit', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    console.log('Submitting delete kelas to:', form.action);
                    fetch(form.action, {
                        method: 'POST',
                    ...
                    .then(response => {
                        console.log('Delete response status:', response.status, 'Delete response URL:', response.url);
                        if (!response.ok) {
    ...
    document.querySelectorAll('.manage-mapel-button').forEach(button => {
        button.addEventListener('click', (event) => {
            event.preventDefault();
            const id = button.getAttribute('data-id');
            const actionUrl = "{{ route('akademik.kelas-semester.mapel.store') }}";
            ...
            console.log('Opening mapel modal with action:', actionUrl, 'kelas_semester_id:', id);
    ...
    document.getElementById('tambah-mapel-form')?.addEventListener('submit', function(event) {
        ...
        const formData = new FormData(this);
        console.log('Submitting mapel form to:', url, 'with data:', Object.fromEntries(formData));
        ...
        .then(response => {
            console.log('Mapel response status:', response.status, 'Mapel response URL:', response.url);
            if (!response.ok) {
                return response.json().then(err => { throw new Error(JSON.stringify(err) || 'Terjadi kesalahan pada server.') });
            }
            return response.json();
        ...
        .catch(error => {
            console.error('Error submitting mapel form:', error);
            Swal.fire('Error!', error.message || 'Terjadi kesalahan pada server.', 'error');
        });
    });
    ...
    document.querySelectorAll('.delete-mapel-form').forEach(form => {
        form.addEventListener('submit', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    console.log('Submitting delete mapel to:', form.action);
                    fetch(form.action, {
                        method: 'POST',
                        body: new FormData(form),
                        headers: {
                    ...
                    .then(response => {
                        console.log('Delete mapel response status:', response.status, 'Delete mapel response URL:', response.url);
                        if (!response.ok) {
                            return response.json().then(err => { throw new Error(JSON.stringify(err) || 'Terjadi kesalahan saat menghapus data.') });
                        }
                        return response.json();
